...
Many summaries of soil profile characteristics can be reported at the observation level. These include a number that have been specifically requested for SLGA II, others from the ASRIS tech spec (v1.6), and others can be derived from diagnostic criteria used in the ASC.
  ...
  * Surface horizon colour, B2 colour (and ASC colour class)
...
By the above, rank 3's should usually key out as SO/CH/KU in the ASC, and many of the rank 2's probably should as well. 
...
athenB <- A_bnd %>%   
  ...
  left_join(., dplyr::select(sit_data[['HOR']], 
                             ...
                             DESIGN_SUBDIV, HORIZON_NAME),
            by = c("PROJECT_CODE", "SITE_ID", "OBS_NO", "HORIZON_NO")) %>% 
  mutate(A_OVER_B2 = case_when(DESIGN_MASTER %in% c('B', 'B?') &
...
B1thenB2 <- B1_bnd %>%   
  ...
  left_join(., dplyr::select(sit_data[['HOR']], 
                             ...
                             DESIGN_SUBDIV, HORIZON_NAME), 
            by = c("PROJECT_CODE", "SITE_ID", "OBS_NO", "HORIZON_NO")) %>% 
  mutate(B1_OVER_B2 = case_when(DESIGN_MASTER %in% c('B', 'B?') &
                                is.na(DESIGN_NUM_PREFIX) &
                              grepl('^2', DESIGN_SUBDIV) ~ TRUE,
                              # unspecified B also allowed 
                              DESIGN_MASTER %in% c('B', 'B?') &
...
tc_rating <- dat %>%
  ...
  rename(TEX_BELOW_BNDRY = TEXTURE) %>% 
  # condense last A/B1
  mutate(TEX_ABOVE_BNDRY = LAST_A_TEX,
         TEX_ABOVE_BNDRY = ifelse(is.na(TEX_ABOVE_BNDRY) & !is.na(LAST_B1_TEX), 
                                  LAST_B1_TEX, TEX_ABOVE_BNDRY))
# now conv to rank, calc jump, rate
