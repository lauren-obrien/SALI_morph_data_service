---
title: "TERN - morphology data for SLGA II - Layer 1"
...
Initial plans are to publish a two-layer web feature service with point geometry:
  ...
  * Layer 2: Horizon-level data relevant to SLGA II modelling 
...
Simple parameters are those where only one record per site or per observation exists, so they're easier just to query out in one go:
...
if(file.exists(l1s_datafile)) {
  l1_simple <- readRDS(l1s_datafile)
} else {
  l1_simple <- 
    dbGetQuery(SALI, 
...
Some site and observation-level parameters can have multiple records, but for Layer 1 they can be reported in a simplified format. These are
...
This has managment implications and is often requested.
...
This one's potentially handy for field ID of soils. Converting to ASC colour classes for simplicity and only using moist colours.
