...
# e.g.
plot(dplyr::filter(sites_layer, PROJECT_CODE == 'BAB')['SLOPE_CLASS'], pch = 16)
```
...
obs_layer <- reduce(obs_data, full_join)
...
# e.g.
plot(dplyr::filter(obs_layer, PROJECT_CODE == 'BAB')['DRAINAGE'], pch = 16)
```
...
# e.g.
bundy_scol <- dplyr::filter(horizon_layer, PROJECT_CODE == 'BAB' & 
                              between(UPPER_DEPTH, 0.25, 0.5) &
                              HOR_MOISTURE_STAT == 'M')

plot(bundy_scol[0], pch = 16, col = bundy_scol$HOR_HEXCODE)
```
...
#e.g.
bundy_toltrees <-  dplyr::filter(vegetation_layer, PROJECT_CODE == 'BAB' &
                                   VEG_STRATA_CODE == 'T')

plot(bundy_toltrees['HEIGHT_CLASS'], pch = 16)
```
