...
Exploring an alternate option where the service structure follows the SALI SITE database structure a bit more closely. Layer 1 will still have the calculated summaries seen in 'PROCESSING_Layer-1.Rmd', but some data like soil classification will be punted out to its own table.
...
A minority of mottles are described with reference to Munsell codes so may as well supply a hex value here too. Nominating some fairly arbitrary hex values for Yellow Book mottle colours as:

  * R Red    - 10R48  - #994730
  * O Orange - 5YR78  - #E89E63
  * B Brown  - 10YR46 - #805921
  * Y Yellow - 5Y88   - #E1C85A
  * G Grey   - 10YR61 - #9E9187
  * D Dark   - 10YR31 - #4F453D
  * L Gley   - 10GY51 - #737D73
  * P Pale   - 10YR81 - #D1C7BD
...
layer_7 <- sit_data[['MOT']] %>% 
  ...
  mutate(HEX_VALUE = 
           case_when(COLOUR_BOOK == 'Y' & COLOUR_CODE == 'R' ~ '994730',
                     ...
                     TRUE ~ HEX_VALUE)) %>% 
  dplyr::select(PROJECT_CODE, SITE_ID, OBS_NO, HORIZON_NO, MOTT_NO,
...
layer_12 <- do.call('rbind', list(sit_data[['FT_PH']], sit_data[['FT_EC']],
  ...
  dplyr::select(-matches('CREAT'), -matches('UPDAT')) %>% 
  dplyr::select(PROJECT_CODE, SITE_ID, OBS_NO, HORIZON_NO, TEST_NO,
                everything()) %>% 
